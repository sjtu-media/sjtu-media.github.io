...
# install runtime tools for cite generation, git metadata, and privilege dropping
RUN apt-get update \
    && apt-get install -y --no-install-recommends git gosu python3 python3-pip \
...
# trust the bind-mounted repo path for git-based plugins
...
COPY scripts/preview-entrypoint.sh /var/entrypoint.sh
